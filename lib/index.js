...
const CORRECT_PROJECT_NAME_REGEX = /^[a-z][a-z0-9-]*$/
    ...
    const config = await prompts(
      [
        ...
        },
        {
          ...
          validate: value => {
            if (!ONE_CHARACTER_REGEX.test(value)) return 'The project must have a name!'

            if (!CORRECT_PROJECT_NAME_REGEX.test(value)) return 'Use lowercase letters, numbers and hyphens only. Must start with a letter.'
        ...
        },
        {
          ...
          choices: [
            {
              title: 'Jest (stable, widely used)',
              value: 'jest'
            },
            {
              title: 'Vitest (fast, modern)',
              value: 'vitest'
            }
          ]
        },
