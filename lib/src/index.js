...
const biome = require('./functions/biome')
...
module.exports = async config => {
  ...
  // Build package list based on test framework
  const testPackages =
    testFramework === 'vitest' ? ['vitest'] : ['jest', 'ts-jest', '@types/jest']
...
${testPackages.join(' ')}`
  ...
  try {
    ...
    const promises = [
      ...
      index(projectName),
      testsF(projectName, testFramework),
 ...
 * @property {'jest'|'vitest'} testFramework test framework to be used
 */
